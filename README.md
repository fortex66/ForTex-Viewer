...
- 테스트 버전이며 앞으로 추가할 사항들은 아래에 적어두었습니다.
...
1. 메인화면 그래프가 다른 페이지를 다녀와도 계속 나오게 백그라운드 실행
2. 설정온도도 선으로 표시되게 하기 - 그래프 색상 변경 삭제 고려
3. 화면 UI 재구성(비율)
4. 실시간 방문자수, 접속자수 표시기능
5. 데이터베이스에 현재온도, 설정온도, 시간, 온도계 상태 4가지가 저장되게 변경
